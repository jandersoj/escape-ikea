...
@export var next_room: String ="res://starbies2.tscn"
...
func _process(delta: float) -> void:
	...
	if(Counter.check>3):
		...
		#
	#if(Counter.check>5&&$"../???".position.y<20):
			#$"../???".rotate(rotation_vector_y, 0.075 * ((randi()%4) + 1))
	#if(Counter.check>6):
		#$"../???".position += rotation_vector_y * .03 
...
##		14 SECONDS total 10 seconds spin
		...
		#$"../???".rotation += rotation_vector_z * .06
		#$"../???".rotation += rotation_vector_x * .1
		#$"../???".rotation += rotation_vector_y * .04
